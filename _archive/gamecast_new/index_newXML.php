			...
			function update_all(){
				$.ajax({
					...
					success: function(xml) {
						...
						$.ajax({
							type: "GET",
							url: "/gamecast_new/xml/play_by_play_extended.xml"+'?'+Math.random(),
							dataType: "xml",
							success: function(pbp) {
								/*var inning_words = ["0","one","two","three","four","five","six","seven","eight","nine","ten"];
								var half_words = ["t","b"];
								var pbp_tagName = inning_words[parseInt($(xml).find('realtime inning_number').text())]+'_play_';
								...
								console.log(    $(pbp).find('inning[number="1"] top play[number="2"]').length     );
